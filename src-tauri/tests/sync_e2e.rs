use kardinal_anty_lib::sync::types::*;
use reqwest::Client;
use serde_json::json;
use std::env;
use std::fs;
    ...
    Ok(response) => {
      if !response.status().is_success() {
        panic!(
          ...
          Server URL: {}\n\
          Please ensure:\n\
          1. MinIO is running (docker compose up -d in kardinal-sync/)\n\
          2. kardinal-sync server is running (cd kardinal-sync && pnpm start:dev)\n\
