...
async fn test_wireguard_tunnel_init() {
  ...
  use kardinal_anty_lib::vpn::{VpnTunnel, WireGuardTunnel};
...
async fn test_openvpn_tunnel_init() {
  ...
  use kardinal_anty_lib::vpn::{OpenVpnTunnel, VpnTunnel};
...
async fn test_tunnel_manager() {
  use kardinal_anty_lib::vpn::{TunnelManager, VpnStatus, VpnTunnel};
  ...
  impl VpnTunnel for MockTunnel {
    async fn connect(&mut self) -> Result<(), kardinal_anty_lib::vpn::VpnError> {
      self.connected = true;
      Ok(())
    }

    async fn disconnect(&mut self) -> Result<(), kardinal_anty_lib::vpn::VpnError> {
