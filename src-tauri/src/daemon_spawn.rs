...
fn get_daemon_path() -> Option<PathBuf> {
  ...
  if let Ok(current_exe) = std::env::current_exe() {
    if let Some(exe_dir) = current_exe.parent() {
      let daemon_path = exe_dir.join("kardinal-daemon");
  ...
  let paths = [
    ...
    dirs::home_dir()
      .map(|h| h.join("Applications/KARDINAL Anty.app/Contents/MacOS/kardinal-daemon"))
      .unwrap_or_default(),
  ];
...
fn get_daemon_path() -> Option<PathBuf> {
  ...
  if let Ok(current_exe) = std::env::current_exe() {
    ...
    let daemon_name = "kardinal-daemon.exe";
    #[cfg(target_os = "linux")]
    let daemon_name = "kardinal-daemon";
  ...
  #[cfg(target_os = "windows")]
  {
    if let Ok(output) = Command::new("where").arg("kardinal-daemon").output() {
  ...
  #[cfg(target_os = "linux")]
  {
    if let Ok(output) = Command::new("which").arg("kardinal-daemon").output() {
      if output.status.success() {
        let path = String::from_utf8_lossy(&output.stdout);
        let path = path.trim();
        if !path.is_empty() {
