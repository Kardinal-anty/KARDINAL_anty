...
This module provides comprehensive self-update functionality for the KARDINAL Anty application
...
impl AppAutoUpdater {
  ...
  pub async fn download_and_prepare_update(
    ...
    let temp_dir = std::env::temp_dir().join("kardinal_app_update");
  ...
  async fn install_update(
    ...
    #[cfg(target_os = "macos")]
    {
      ...
      // Clean up old "KARDINAL Anty.app" if it exists (from before the project rename)
      if let Some(parent_dir) = current_app_path.parent() {
        let old_app_path = parent_dir.join("KARDINAL Anty.app");
        if old_app_path.exists() && old_app_path != current_app_path {
          log::info!(
            "Removing old 'KARDINAL Anty.app' from: {}",
            old_app_path.display()
          );
          if let Err(e) = fs::remove_dir_all(&old_app_path) {
            log::warn!("Warning: Failed to remove old 'KARDINAL Anty.app': {e}");
          } else {
            log::info!("Successfully removed old 'KARDINAL Anty.app'");
          }
        }
      }
  ...
  async fn restart_application(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    #[cfg(target_os = "macos")]
    {
      ...
      let script_path = temp_dir.join("kardinal_restart.sh");
...
"#,
    ...
    #[cfg(target_os = "windows")]
    {
      ...
      let temp_dir = std::env::temp_dir();
      let script_path = temp_dir.join("kardinal_restart.bat");
...
mod tests {
  ...
  fn test_platform_specific_download_urls() {
    ...
    let all_assets = vec![
      // macOS assets
      AppReleaseAsset {
        name: "KARDINAL.Anty_0.1.0_aarch64.dmg".to_string(),
        browser_download_url: "https://example.com/aarch64.dmg".to_string(),
        size: 12345,
      },
      AppReleaseAsset {
        name: "KARDINAL.Anty_0.1.0_x64.dmg".to_string(),
      ...
      // Linux assets
      AppReleaseAsset {
        name: "kardinal-anty_0.1.0_amd64.deb".to_string(),
        browser_download_url: "https://example.com/amd64.deb".to_string(),
        size: 12345,
      },
      AppReleaseAsset {
        name: "kardinal-anty-0.1.0-1.x86_64.rpm".to_string(),
        browser_download_url: "https://example.com/x86_64.rpm".to_string(),
        size: 12345,
      },
      AppReleaseAsset {
        name: "KARDINAL.Anty-0.1.0-x86_64.AppImage".to_string(),
