...
use kardinal_anty_lib::proxy_runner::{
  start_proxy_process_with_profile, stop_all_proxy_processes, stop_proxy_process,
};
use kardinal_anty_lib::proxy_server::run_proxy_server;
use kardinal_anty_lib::proxy_storage::get_proxy_config;
use std::process;
...
async fn main() {
  ...
  let matches = Command::new("kardinal-proxy")
    .subcommand(
      Command::new("proxy")
  ...
  if let Some(proxy_matches) = matches.subcommand_matches("proxy") {
    if let Some(start_matches) = proxy_matches.subcommand_matches("start") {
      ...
      if let Some(id) = stop_matches.get_one::<String>("id") {
        ...
        let configs = kardinal_anty_lib::proxy_storage::list_proxy_configs();
