    ...
    try {
      ...
      const location =
        locationParts.length > 0
          ? locationParts.join(", ")
          : t("proxies.check.unknown");

      toast.success(
        <div className="flex flex-col">
          {t("proxies.check.locationIs")}
      ...
      toast.error(t("proxies.check.failedError", { error: errorMessage }));
        ...
        {isCurrentlyChecking ? (
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-primary-foreground/70">
              {t("proxies.check.ip", { ip: result.ip })}
            </p>
            <p className="text-xs text-primary-foreground/70">
              {t("proxies.check.checked", {
          ...
          <div>
            <p>{t("proxies.check.checkFailed")}</p>
            <p className="text-xs text-primary-foreground/70">
              {t("proxies.check.failedTime", {
                time: formatRelativeTime(result.timestamp),
              })}
            </p>
          </div>
        ) : (
          <p>{t("proxies.check.checkValidity")}</p>
        )}
      </TooltipContent>
    </Tooltip>
  );
